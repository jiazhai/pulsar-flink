...
public class PulsarPrimitiveSchema<T> implements PulsarSerializationSchema<T>,
    ...
    public void serialize(T element, TypedMessageBuilder<byte[]> messageBuilder) {
        messageBuilder.value(targetSchema.encode(element));
    }

    @Override
    public void deserialize(Message<?> message, Collector<T> out) throws IOException {
        out.collect(targetSchema.decode(message.getData()));
    }
